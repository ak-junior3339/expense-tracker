...
/* ---- Danger zone — no background, just a top border separator ---- */
.danger-box {
...
with st.sidebar:
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"### 💸 खर्चा")
    st.markdown("<hr style='border-color:#1e1e3a;margin:0.5rem 0'>", unsafe_allow_html=True)

    if st.session_state.is_admin:
        st.markdown(f"**{st.session_state.username}**")
        st.markdown('<span class="admin-chip">ADMIN</span>', unsafe_allow_html=True)
    else:
        st.markdown(f"👤 **{st.session_state.username}**")
        st.caption("Your data is private to you.")

    st.markdown("<br>", unsafe_allow_html=True)

    if st.session_state.is_admin:
        menu_choice = "Admin Panel"
        st.markdown("🔒 **Admin Panel**")
    else:
        menu_choice = st.selectbox(
            "Navigate",
            ["➕ Add Expense", "📋 View Expenses", "📊 Reports"],
...
if menu_choice == "➕ Add Expense":
    ...
    st.markdown("""
        <div class="page-banner">
            <div>
                <div class="sub">TRACK YOUR MONEY</div>
                <h1>Add Expense</h1>
            </div>
...
# ================================================================
    ...
    st.markdown("""
        <div class="page-banner">
            <div>
                <div class="sub">YOUR HISTORY</div>
...
# ================================================================
    ...
    # BUG FIX: avoid divide-by-zero on day 0 (shouldn't happen, but safe)
    ...
    # TAB 1 — Bar: daily spending this month
    with tab1:
        if current_month.empty:
            ...
            daily = (
                current_month
                .groupby(current_month["Date"].dt.day)["Amount"]
                .sum().reset_index()
            )
    ...
    with tab2:
        ...
        if type_data.empty:
            ...
            fig_pie = px.pie(
                type_data, names="Category", values="Amount",
                hole=0.42,
            ...
            fig_pie.update_layout(
                ...
                annotations=[dict(
                    text=f"<b>₹{df['Amount'].sum():,.0f}</b><br>total",
    ...
    with tab3:
        ...
        if mode_data.empty:
            ...
            fig_donut = px.pie(
                mode_data, names="Mode", values="Amount",
                hole=0.6,
            ...
            fig_donut.update_layout(
                ...
                annotations=[dict(
                    text=f"<b>₹{df['Amount'].sum():,.0f}</b><br>total",
                    x=0.5, y=0.5, font_size=16, font_color="#1a1a2e",
                    showarrow=False
                )]
            )
    ...
    with tab4:
        ...
        if hmap_df.empty:
            ...
            while day_num <= num_days:
                ...
                day_num += 1
                if len(week_z) == 7:
                    grid_z.append(week_z)
                    grid_text.append(week_text)
                    week_z, week_text = [], []
            ...
            fig_hmap.update_layout(
                xaxis=dict(
                    ...
                    side="top", tickfont=dict(size=12, family="Sora")
                ),
                ...
                title=dict(
                    text=f"<b>{datetime.date(h_year, h_month, 1).strftime('%B %Y')}</b>",
                    x=0.5, font=dict(size=14, family="Sora")
                )
            )
    ...
    monthly["Period"] = monthly.apply(
        lambda r: datetime.date(int(r["Year"]), int(r["Month"]), 1).strftime("%b %Y"), axis=1
    )
    ...
    if len(monthly) < 2:
        ...
        fig_line = px.line(
            monthly, x="Period", y="Amount", markers=True,
            labels={"Period": "Month", "Amount": "Total Spent (₹)"},
            line_shape="spline",
        )
    ...
    with br_col1:
        # BUG FIX: unique variable name so no conflict with heatmap selectbox
    ...
    filtered = df[
        (df["Date"].dt.month == br_month) &
        (df["Date"].dt.year  == br_year)
    ]
    ...
    if filtered.empty:
        ...
        st.dataframe(
            filtered.drop(columns=["ID"]).reset_index(drop=True),
            width='stretch', hide_index=True
        )
        st.markdown(f"""
